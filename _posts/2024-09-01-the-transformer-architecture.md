...
3. **Scaled Dot-Product Attention**
    - Similarity measures are computed $Q \cdot K^T$ resulting in a set of raw attention scores (for each key in the sequence). These are scaled (to combat tiny gradients) and passed through a softmax function $softmax(\frac{QK^T}{\sqrt{d_k}})$ producing a probability distribution, with scores now representing how much attention to give to each key's corresponding value. This scaling factor can instead be a trainable parameter if setting `use_scale=True`.
    - A weighted sum of the values is produced $\sum\limits_{i}(\text{Attention Weight}_i \times V_i)$ by multiplying each value by its corresponding attention weight, and summing the results.
